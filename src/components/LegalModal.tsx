...
/* ── Guest Contact Form ── */
function GuestContactForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)] mb-1.5">Name</label>
        <input
          type="text"
          value={name}
          onChange={(e) => setName(e.target.value)}
          required
          className="w-full bg-[var(--background)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--muted-foreground)]/40 focus:outline-none focus:border-[var(--color-gold)]/50 transition-colors"
          placeholder="Your name"
        />
      </div>
      <div>
        <label className="block text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)] mb-1.5">Email</label>
        <input
          type="email"
          value={email}
      ...
      <div>
        <label className="block text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)] mb-1.5">Message</label>
        <textarea
          value={message}
          onChange={(e) => setMessage(e.target.value)}
          required
          rows={4}
          className="w-full bg-[var(--background)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--muted-foreground)]/40 focus:outline-none focus:border-[var(--color-gold)]/50 transition-colors resize-none"
      ...
      <button
        ...
        className="w-full py-2.5 rounded-xl bg-[var(--color-gold)] text-[#0A0A0A] font-bold text-xs uppercase tracking-widest hover:opacity-90 transition-opacity disabled:opacity-30"
      >
...
export default function LegalModal({ isOpen, onClose }: Props) {
  ...
  }, [isOpen, onClose, showContactForm]);
    ...
    <>
      ...
      >
        ...
        >
          {/* Close button */}
          <button
            onClick={() => showContactForm ? setShowContactForm(false) : onClose()}
          ...
          <div className="overflow-y-auto px-6 py-6 space-y-8" style={{ maxHeight: 'min(80vh, 700px)' }}>
            ...
            {/* ── Guest Contact Form overlay ── */}
            {showContactForm ? (
              <section>
                <h3 className="font-serif text-sm font-bold uppercase tracking-widest text-gold mb-4">Contact JazzNode HQ</h3>
                {contactSent ? (
                  <div className="text-center py-8 space-y-3">
                    ...
                    <p className="text-sm text-[var(--foreground)] font-semibold">Message sent!</p>
                    <p className="text-xs text-[var(--muted-foreground)]">We&apos;ll get back to you within 24 hours.</p>
                    <button
                      type="button"
                      onClick={onClose}
                      className="mt-2 px-6 py-2 rounded-xl border border-[var(--border)] text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors"
                    >
                      Close
                    </button>
                  </div>
                ) : (
                  <GuestContactForm onSuccess={() => setContactSent(true)} />
                )}
              </section>
            ) : (
              <>
