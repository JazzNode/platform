...
export default function VenueBadgeShowcase({ venueId }: { venueId: string }) {
    ...
    (async () => {
      try {
        ...
        const [{ data: venue }, { data: events }] = await Promise.all([
          supabase.from('venues').select('tier, jazz_frequency, friendly_en, friendly_ja, friendly_ko, friendly_th, friendly_id').eq('venue_id', venueId).single(),
          supabase.from('events').select('event_id, tag_list').eq('venue_id', venueId).limit(500),
        ]);
        ...
        for (const badge of allBadges) {
          ...
          switch (bid) {
            case 'ven_genre_explorer':
              isEarned = tagSet.size >= (target || 5); break;
            case 'ven_marathon':
              isEarned = (events || []).length >= (target || 20); break;
            case 'ven_multilingual':
              isEarned = friendlyLangCount >= (target || 2); break;
            case 'ven_jazz_hub':
              isEarned = venue?.jazz_frequency === 'nightly'; break;
            case 'ven_house_keys':
              isEarned = !!(venue?.tier && venue.tier >= 1); break;
          }
  ...
  return (
    <Link
      href={`/${locale}/profile/venue/${venueId}/badges`}
