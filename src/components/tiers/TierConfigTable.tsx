  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && data[entityType]?.features) {
          setFeatures(data[entityType].features);
          setOriginalFeatures(data[entityType].features);
        }
        if (!cancelled && data[entityType]?.visible_tiers) {
          setVisibleTiers(data[entityType].visible_tiers);
          setOriginalVisibleTiers(data[entityType].visible_tiers);
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/admin/tier-config', {
        ...
        body: JSON.stringify({ entityType, features, visibleTiers }),
      });
      if (res.ok) {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-3xl font-bold">{t(titleKey)}</h1>
          <p className="text-sm text-[var(--muted-foreground)] mt-1">{t(descKey)}</p>
        </div>
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 space-y-4">
        ...
        {FEATURES.filter((f) => (features[f.key] ?? 0) < 0).length > 0 && (
          <div className="space-y-3 pb-3 border-b border-[var(--border)]">
            ...
            {FEATURES.filter((f) => (features[f.key] ?? 0) < 0).map((feat) => (
