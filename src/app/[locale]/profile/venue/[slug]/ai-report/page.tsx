...
interface ReportData {
  kpis: {
    ...
    events?: { current: number; change: number };
...
export default function VenueAIReportPage() {
  ...
  // Resolve slug → venue_id
  useEffect(() => {
    if (!slug) return;
    import('@/utils/supabase/client').then(({ createClient }) => {
      const supabase = createClient();
      supabase.from('venues').select('venue_id').eq('venue_id', slug).single()
        .then(({ data }) => setVenueId(data?.venue_id || slug));
    });
  }, [slug]);
  ...
  const fetchReport = useCallback(async (id?: string) => {
    ...
    try {
      const url = id
        ? `/api/venue/ai-report?venueId=${venueId}&id=${id}`
        : `/api/venue/ai-report?venueId=${venueId}`;
  ...
  const fetchHistory = useCallback(async () => {
    if (!token || !venueId) return;
    try {
      const res = await fetch(`/api/venue/ai-report?venueId=${venueId}&history=true`, {
  ...
  }, [token, venueId, fetchReport, fetchHistory]);
    ...
    <div className="max-w-4xl">
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)] mt-1">場地過去 30 天數據摘要與 AI 分析建議</p>
        </div>
        ...
        <div className="space-y-8">
          <section>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              {data.kpis.events && (
                <div className="bg-[var(--muted)] rounded-xl p-4 flex flex-col gap-2">
                  <p className="text-xs text-[var(--muted-foreground)]">本月演出</p>
                  <p className="text-2xl font-bold tabular-nums">{data.kpis.events.current.toLocaleString()}</p>
                  <ChangeChip change={data.kpis.events.change} />
                </div>
              )}
            </div>
          </section>
