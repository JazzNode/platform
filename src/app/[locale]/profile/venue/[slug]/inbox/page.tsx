          ...
          <div className="space-y-3">
            {unreadNotifs > 0 && (
              <div className="flex justify-end">
                <button onClick={markAllNotifsRead} className="text-xs text-emerald-400 hover:underline">
                  {tInbox('markAllRead')}
                </button>
              </div>
            )}
            {notifsLoading ? (
              ...
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-8 text-center">
                <p className="text-sm text-[var(--muted-foreground)]">{tInbox('noNotifications')}</p>
              </div>
            ) : (
