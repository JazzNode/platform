...
function StatCard({ value, suffix, label }: { value: number; suffix?: string; label: string }) {
  return (
    <div className="text-center">
      <div className="font-serif text-3xl sm:text-4xl font-bold text-[var(--color-gold)]">
        <CountUp end={value} trigger="visible" />
        {suffix && <span>{suffix}</span>}
      </div>
      <p className="text-xs sm:text-sm text-[var(--muted-foreground)] mt-1 uppercase tracking-wider">{label}</p>
    </div>
  );
}
      ...
      <FadeUp>
        <section className="border-y border-[var(--border)] py-10 mb-24">
          <div className="max-w-4xl mx-auto grid grid-cols-3 gap-6 px-4">
            <StatCard value={200} suffix="+" label="Cities" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {moreFeatures.map((feat, i) => (
            <FadeUpItem key={i} delay={i * 80}>
              <div className="group bg-[var(--card)] border border-[var(--border)] rounded-xl p-6 text-center hover:border-[var(--color-gold)]/20 transition-colors duration-500">
